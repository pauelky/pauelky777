...
def build_start_text(
    ...
    *,
    ...
    voice_state = "голос включён" if user_obj.get("voice", False) else "голос выключен"
    chat_title = _normalize_chat_title_text(active_chat_title, fallback="Основной чат") if active_chat_title else "Основной чат"
    role_label = _get_public_reply_style_label(current_role or user_obj.get("reply_style") or "neutral")
    image_engine = _format_public_image_engine_name(globals().get("STABILITY_ENGINE_ID"))
    return (
        "Привет! Я твой умный помощник в Telegram 🤖\n\n"
        "Здесь можно быстро перейти в нужный режим:\n"
        "• вести полноценный диалог с ИИ\n"
        "• переключаться между чатами и хранить историю\n"
        "• разбирать фото, скриншоты и текст с изображений\n"
        "• генерировать изображения по описанию\n"
        "• менять подачу ответа под задачу\n\n"
        "Сейчас активно:\n"
        f"• чат: {chat_title}\n"
        f"• режим ответа: {role_label}\n"
        f"• голосовые ответы: {voice_state}\n"
        f"• генерация изображений: {image_engine}\n\n"
        "Выбери действие в меню ниже или просто отправь сообщение, фото или голосовое."
    )
...
def build_help_text() -> str:
    return (
        "✨ Возможности бота\n\n"
        "💬 Диалог с ИИ\n"
        "Открывает живой диалог внутри текущего чата. Можно общаться текстом или голосом.\n\n"
        "🗂 Мои чаты\n"
        "Позволяет создавать отдельные чаты, переключаться между ними, переименовывать и смотреть историю сообщений.\n\n"
        "🖼 Разобрать фото\n"
        "Отправь фото или скриншот, и я постараюсь аккуратно распознать текст, понять задачу и помочь по смыслу.\n\n"
        "🎨 Создать изображение\n"
        "Генерирует картинку по описанию. Чем точнее промпт, тем лучше результат.\n\n"
        "🪄 Режим ответа\n"
        "Меняет стиль подачи: коротко, подробно, как учитель, как аналитик, как редактор и не только.\n\n"
        "🌦 Погода\n"
        "Показывает прогноз по городу и быстрые погодные сценарии.\n\n"
        "🧠 Факт дня\n"
        "Выдаёт короткий интересный факт, когда хочется чего-то лёгкого и полезного.\n\n"
        "Полезные команды:\n"
...
def build_chat_manager_text(user_id: int, page: int = 0) -> str:
    ...
    return (
        "🗂 Мои чаты\n\n"
        "Здесь удобно держать отдельные диалоги по темам: работа, идеи, учёба, личные вопросы и любые другие сценарии.\n\n"
        "Сейчас активно:\n"
        f"• чат: {active_title}\n"
        f"• всего чатов: {total}\n\n"
        "Ниже список твоих чатов. Можно открыть карточку, выбрать активный чат или создать новый."
    )
